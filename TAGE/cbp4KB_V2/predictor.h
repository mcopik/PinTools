#ifndef _PREDICTOR_4KBV2_H_
#define _PREDICTOR_4KBV2_H_
...
int m[NHIST + 1]= {0,4,13,24,53,91}; // history lengths
int TB[NHIST+1]= {0,7,7,8,10,10};		// tag width for the different tagged tables
int logg[NHIST + 1]={0,10,10,9,9,8};		// log of number entries of the different tagged tables
...
int UsedHistory;
  PREDICTOR (void)
  {
       predictorsize ();    reinit ();
  }
  void flush(int percentage)
  {
    srand(1410);
    //each table
    for(int i = 0; i <= NHIST; ++i) {
      int hist_size = 1 << logg[i];
      int cut_part = static_cast<int>(percentage / 100.0 * hist_size);
      for(int j = 0; j < cut_part; ++j) {
        float pos = ((double)rand())/((double)RAND_MAX);
        int int_pos = static_cast<int>(pos * hist_size);
        if(int_pos >= hist_size)
          abort();
        gtable[i][int_pos].tag = 0;
      }
    }
  }
  void reinit ()
...
//TAGE UPDATE
      ...
      if ((IsStatistic) & (pred_Statistic == resolveDir))       ALLOC = false;      

if (pred_taken == resolveDir) 	if ((MYRANDOM () & 31) != 0) 	  ALLOC = false;
...
void CheckStatistic (uint32_t pc, bool pred)
  ...
  void UpdateStatistic (uint32_t pc, bool pred, bool taken)
  {
	...
	if ((abs (2 * Statistic[indexStatistic].ctr + 1) == 1)
	    || ((Statistic[indexStatistic].ctr >= 0) == pred))
	  {
